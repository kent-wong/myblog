...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            # 如果是从其他页面redirect到登录页，登录成功后再跳转回去
...
def new_post():
    ...
    return render_template('create_post.html', title='新帖子', form=form, legend='新帖子')
...
def update_post(post_id):
    ...
    return render_template('create_post.html', title='修改帖子', form=form, legend='修改帖子')
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('邮件已经发出，请按照邮件中的指示进行操作', 'info')
        return redirect(url_for('login'))
